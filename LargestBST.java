...
class Main {
	static class BinaryTree {
		...
		private Info largestBSTinBT(Node root) {
			...
			Info x=largestBSTinBT(root.left);
			Info y=largestBSTinBT(root.right);
			Info res=new Info();
			res.size=x.size+y.size+1;
			res.min=Math.min(root.data,Math.min(x.min,y.min));
			res.max=Math.max(root.data,Math.max(x.max,y.max));

			if(x.isBST && y.isBST && x.max<root.data && y.min>root.data){
				res.isBST=true;
				res.ans=x.size+y.size+1;
			}
			else{
				res.isBST=false;
				res.ans=Math.max(y.ans,x.ans);
			}
			return res;
		}
